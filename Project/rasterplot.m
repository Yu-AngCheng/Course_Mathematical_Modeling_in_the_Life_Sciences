...
function rasterplot(times,numtrials,triallen, color,varargin)
...
%%%%%%%%% Code Begins %%%%%%%%%%%%
switch nin
 case 4 %no handle so plot in a separate figure
  figure;
  hresp=gca;
  fs=defaultfs;
 case 5 %handle supplied
  hresp=varargin{1};
  if (~ishandle(hresp))
 ...
 % plot spikes
  ...
  axes(hresp);
  h=plot(xx, yy, plotcolor, 'linewidth',plotwidth,'Color',color);
